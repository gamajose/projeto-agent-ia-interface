...
_TERMINAL = {"resolved"}
_ACTIVE = {"new", "queued", "investigating", "awaiting_approval", "watching", "needs_attention"}
...
def _autonomy_eligible(incident: dict[str, Any], result: dict[str, Any], settings: Settings) -> tuple[bool, str]:
    ...
    if str(incident.get("environment") or "unknown") not in {"monitoring", "training"}:
        return False, "ambiente fora do envelope autônomo"
    ...
    proposals = [
        item for item in (analysis.get("proposed_actions") or [])
        if isinstance(item, dict) and item.get("status") == "proposed"
    ]
    if not proposals:
        return False, "nenhuma ação corretiva proposta"
    requested = {str(item.get("tool") or "") for item in proposals}
...
def _auto_execute(
    ...
    for round_number in range(1, max_rounds + 1):
        ...
        pending_tools = {str(item.get("tool") or "") for item in pending}
...
def _record_communications(
    ...
    *,
    ...
    communications = build_incident_communications(incident, result, state=incident.get("status"), settings=settings)
...
def postprocess_investigation_result(
    ...
    *,
    ...
    try:
        ...
        autonomy = {"eligible": True, "status": "failed", "reason": f"{type(exc).__name__}: {exc}"}
    ...
    if execution:
        ...
        incident = _incident_update(
            incident_id,
            {
                ...
                "status": "watching" if execution.get("status") == "validated" else "needs_attention",
                "attention_reason": None if execution.get("status") == "validated" else "Self-healing executado, mas a pós-validação local não confirmou a recuperação.",
...
def _reinvestigate(incident: dict[str, Any], runtime: dict[str, Any], settings: Settings) -> dict[str, Any]:
    ...
    objective = (
        ...
        f"Estado atual no Livestatus: {runtime.get('state')} / {runtime.get('status')}. Saída: {runtime.get('plugin_output') or incident.get('last_output')}. "
...
def supervisor_tick(*, settings: Settings | None = None) -> dict[str, Any]:
    ...
    for incident in data.get("items") or []:
        try:
            ...
            errors.append({"incident_id": str(incident.get("id") or ""), "error": f"{type(exc).__name__}: {exc}"})
    return {"enabled": True, "processed": processed, "open": len(data.get("items") or []), "errors": errors[-20:]}
