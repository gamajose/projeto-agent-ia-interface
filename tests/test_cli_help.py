...
def test_full_help_lists_all_operational_commands() -> None:
    ...
    required = (
        "agent --menu", "agent ALVO [PROBLEMA...]", "agent replay UUID", "agent approve UUID TOKEN",
        "agent doctor ai", "agent --version", "--ambiente, -a", "--porta, -p", "--modo",
...
@pytest.mark.parametrize(("argument", "expected"), [("--version", "Agent IA Infra 1.46.10"), ("--help", "AGENT IA INFRA")])
def test_help_and_version_do_not_require_database_configuration(argument: str, expected: str) -> None:
    ...
    result = subprocess.run([sys.executable, "-m", "app.cli.entrypoint", argument], cwd=PROJECT_ROOT, env=environment, capture_output=True, text=True, check=False)
